...
" 加载全局标记注释
function! GlobalLoadMarkComments()
...
nnoremap <leader>svm :call SaveGlobalMarkComments()<cr> \| :call SetProjectViminfo()<cr>
                                            " global: 重置当前环境的 viminfo(切换新项目时)
...
nnoremap <silent> <leader>amx :call DeleteMarkComment(input('Mark: '))<CR>| " 辅助: 删除某一个标记注释
                                            " global：删除标记注释
nnoremap <silent> <leader>smm :call DeleteMarks(input('Enter marks to delete (space-separated): '))<CR>
                                            " 手动删除一系列的标记
